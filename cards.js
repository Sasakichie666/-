...
function detectCombos(cardIds) {
    ...
    if (has('m09_mono') && bunCards.length > 0) rawCombos.push({ name: '对酒当歌', score: 1, cards: [bunCards[0], 'm09_mono'] });
    ...
    if (hasName('花魁') && has('yokku') && (has('tanzaku_spring') || has('tanzaku_summer') || has('tanzaku_autumn') || has('tanzaku_winter'))) {
       rawCombos.push({
           name: '文艺',
           score: 2,
           cards: ['oiran_oiran_01', 'yokku', 'tanzaku_spring', 'tanzaku_summer', 'tanzaku_autumn', 'tanzaku_winter', '【花魁，祝句，任意一张四季文牌】']
       });
    }
