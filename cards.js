...
function detectCombos(cardIds) {
    ...
    if (has('toy_dog_01') && has('temari') && hasName('达摩') && has('hagoita')) {
        combos.push({ name: '四玩具', score: 3, cards: ['toy_dog_01', 'temari', 'toy_daruma_01', 'hagoita'] });
    }

    // 神社初诣
    if (has('shrine_torii') && has('komainu') && has('shrine_lantern') && has('shrine_ema')) {
        combos.push({ name: '神社初诣', score: 4, cards: ['shrine_torii', 'komainu', 'shrine_lantern', 'shrine_ema'] });
    }

    // 花魁道中
    if (hasName('花魁') && has('oiran_umbrella') && has('oiran_geta') && has('oiran_fan')) {
        combos.push({ name: '花魁道中', score: 6, cards: ['oiran_oiran_01', 'oiran_umbrella', 'oiran_geta', 'oiran_fan'] });
    }
    ...
    if (hasName('盆栽') && has('inner_armor') && has('inner_screen')) {
        combos.push({ name: '内室', score: 3, cards: ['inner_bonsai_01', 'inner_armor', 'inner_screen'] });
    }
    ...
    if (hasName('花魁') && has('oiran_umbrella') && has('m05_mono_01') && has('m05_mono_02'))
        combos.push({ name: '伞下雨桥', score: 5, cards: ['oiran_oiran_01', 'oiran_umbrella', 'm05_mono_01', 'm05_mono_02'] });
    ...
    if (hasName('花魁') && has('koto') && has('fune')) combos.push({ name: '乐艺', score: 2, cards: ['oiran_oiran_01', 'koto', 'fune'] });

    if (hasName('花魁') && has('yokku')) {
    // 文艺组合所需文牌包含所有四季文牌（春·歌、夏·句、秋·吟、冬·诗）
        combos.push({
            name: '文艺',
            score: 2,
            cards: ['oiran_oiran_01', 'yokku',  'tanzaku_spring', 'tanzaku_summer', 'tanzaku_autumn', 'tanzaku_winter', '【花魁+祝句+任意一春夏秋冬文牌】']
        });
    }
    ...
    if (yanagi && has('kaze')) combos.push({ name: '柳间风', score: 2, cards: [yanagi, 'kaze'] });
    ...
    if (hasName('达摩') && has('shrine_omamori') && has('toy_crane_01') && has('shrine_ema'))
        combos.push({ name: '四福', score: 3, cards: ['toy_daruma_01', 'shrine_omamori', 'toy_crane_01', 'shrine_ema'] });
    if (hasName('挂轴') && has('inner_armor'))
        combos.push({ name: '文武', score: 2, cards: ['kakejiku_01', 'inner_armor'] });

    if (has('koi') && has('kanagawa')) combos.push({ name: '鲤跃川', score: 2, cards: ['koi', 'kanagawa'] });
    if (has('koi') && has('garden_pond')) combos.push({ name: '游鲤', score: 2, cards: ['koi', 'garden_pond'] });
    if (has('koi') && has('kingyo')) combos.push({ name: '双鱼佩', score: 1, cards: ['koi', 'kingyo'] });
    if (has('hotaru') && has('m06_kemono')) combos.push({ name: '萤蝶舞', score: 1, cards: ['hotaru', 'm06_kemono'] });
    if (has('garden_lantern') && has('komainu')) combos.push({ name: '石造', score: 1, cards: ['garden_lantern', 'komainu'] });
    if (has('garden_lantern') && has('hotaru')) combos.push({ name: '萤火', score: 2, cards: ['garden_lantern', 'hotaru'] });
    if (has('hanabi') && has('kingyo')) combos.push({ name: '绚烂泡影', score: 2, cards: ['hanabi', 'kingyo'] });
    if (has('furin') && has('uchiwa') && has('kingyo')) combos.push({ name: '夏之箱', score: 3, cards: ['furin', 'uchiwa', 'kingyo'] });
...
function getTheoreticalCombosForCard(cardId) {
    // ★ 核心修复：复制牌统一映射到母本 ID，保证明细与母本完全一致
