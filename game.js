...
const GameState = {
    ...
    myHand: [],
    myMelded: [],
    selectedHandCard: null,          // 用于配对操作的手牌 (红框)
    selectedCardCombos: [],          // 选中手牌的理论组合列表（保留兼容）
    phase: 'waiting',
    koikoiPending: false,
    isHost: false,
    peer: null,
    ...
    // 用于组合明细查看的选中牌（手牌或场上牌） (绿框)
    selectedDetailCard: null,
    selectedDetailCombos: [],       // 对应组合列表
};
...
function onHandClick(cardId) {
    ...
    const myTurn = (GameState.players[GameState.currentPlayerIndex] === GameState.myPlayerId);
    const koikoi = GameState.koikoiPending;

    if (myTurn && !koikoi) {
        // 己方正常回合：保留配对操作，同时更新明细查看
        const prev = GameState.selectedHandCard;
        if (prev === cardId) {
            // 再次点击同一张牌，取消选中（配对和明细都清除）
            GameState.selectedHandCard = null;
            GameState.selectedCardCombos = [];
            GameState.selectedDetailCard = null;
            GameState.selectedDetailCombos = [];
            AudioEngine.uiClick();
        } else {
            // 选中新手牌用于配对，同时设置明细查看为同一张牌
            GameState.selectedHandCard = cardId;
            const combos = getTheoreticalCombosForCard(cardId);
            GameState.selectedCardCombos = combos;
            GameState.selectedDetailCard = cardId;
            GameState.selectedDetailCombos = combos;
            AudioEngine.cardSelect();
        }
    } else {
        // 非己方回合或 koikoi 状态：只能查看明细 (绿框)
        // 清除配对状态
        GameState.selectedHandCard = null;
        GameState.selectedCardCombos = [];

        const prevDetail = GameState.selectedDetailCard;
        if (prevDetail === cardId) {
            // 点击同一张牌，取消明细查看
            GameState.selectedDetailCard = null;
            GameState.selectedDetailCombos = [];
            AudioEngine.uiClick();
            ...
            GameState.selectedDetailCard = cardId;
            GameState.selectedDetailCombos = getTheoreticalCombosForCard(cardId);
            AudioEngine.uiClick();
        }
    }
...
function onFieldClick(fieldCardId, index) {
    ...
    // 情况1：己方回合、未koikoi、且已选中手牌，执行配对
    if (myTurn && !koikoi && GameState.selectedHandCard) {
        if (!canPair(GameState.selectedHandCard, fieldCardId)) {
            addLog('不能配对');
            // 配对不成功，但可以顺便查看该场上牌的明细（保留配对状态，同时更新明细）
            GameState.selectedDetailCard = fieldCardId;
            GameState.selectedDetailCombos = getTheoreticalCombosForCard(fieldCardId);
            renderRightSidebar();
            renderField();
            renderHand();
            return;
    ...
    // 情况2：查看场上牌的明细（任何回合均可）(绿框)
    // 清除配对状态
    GameState.selectedHandCard = null;
    GameState.selectedCardCombos = [];

    const prevDetail = GameState.selectedDetailCard;
    if (prevDetail === fieldCardId) {
        GameState.selectedDetailCard = null;
        GameState.selectedDetailCombos = [];
        AudioEngine.uiClick();
    } else {
...
function discardCard(cardId) {
    ...
    if (GameState.players[GameState.currentPlayerIndex] !== GameState.myPlayerId) return;
    GameState.selectedHandCard = null;
    GameState.selectedCardCombos = [];
    GameState.selectedDetailCard = null;
    GameState.selectedDetailCombos = [];
...
function renderField() {
    ...
    GameState.fieldCards.forEach((fc, i) => {
        ...
        // 高亮规则：
        // 1. 如果当前有选中的手牌并且该场地牌可配对 → can-pair (红框)
        if (GameState.selectedHandCard && canPair(GameState.selectedHandCard, topId)) {
            top.classList.add('can-pair');
        }
        // 2. 如果是正在查看明细的场地牌，且没有处于配对状态（即不是可配对的牌或者没有选中手牌）
        if (GameState.selectedDetailCard === topId &&
            !(GameState.selectedHandCard && canPair(GameState.selectedHandCard, topId))) {
            top.classList.add('detail-selected');
        }
...
function renderHand() {
    ...
    GameState.myHand.forEach(id => {
        ...
        // 己方回合选中用于配对的手牌 → selected (红框)
        if (id === GameState.selectedHandCard) {
            el.classList.add('selected');
        }
        // 查看明细的手牌 (非配对操作，且是当前明细牌) → detail-selected (绿框)
        // 注意：如果该牌同时是 selectedHandCard，则不添加绿框，红框优先
        if (GameState.selectedDetailCard === id && GameState.selectedHandCard !== id) {
    ...
    if (rightToggleBtn) {
        rightToggleBtn.style.display = GameState.selectedDetailCard ? 'block' : 'none';
    }
}
...
function updateSelectedCardCombosDisplay() {
    ...
    if (!el) return;
    if (GameState.selectedDetailCard && GameState.selectedDetailCombos.length > 0) {
        const def = getCardDef(GameState.selectedDetailCard);
        const cardName = def ? def.name : '未知';
        const combosText = GameState.selectedDetailCombos.map(c => `${c.name}(${c.score}分)`).join('、');
        el.textContent = `【${cardName}】可参与组合: ${combosText}`;
        el.style.color = '#e8c84c';
    } else if (GameState.selectedDetailCard && GameState.selectedDetailCombos.length === 0) {
        const def = getCardDef(GameState.selectedDetailCard);
        const cardName = def ? def.name : '未知';
        el.textContent = `【${cardName}】暂无组合`;
        el.style.color = '#ff8c8c';
    } else {
        el.textContent = '点击手牌或场上牌查看可参与的组合';
...
function renderRightSidebar() {
    ...
    if (!GameState.selectedDetailCard || GameState.selectedDetailCombos.length === 0) {
        sidebar.innerHTML = '<p style="color:#a09080;">请点击手牌或场上牌查看组合明细</p>';
        return;
    }
    ...
    for (const combo of allCombos) {
        ...
        if (combo.cards && combo.cards.length > 0) {
            if (combo.cards.includes(GameState.selectedDetailCard)) {
                covered = combo.cards;
            }
        } else {
            covered = getCoveredCardsForCombo(combo, allCardIds);
            if (!covered.includes(GameState.selectedDetailCard)) {
                covered = [];
            }
        }
