...
const GameState = {
    ...
    selectedDetailCard: null,       // 当前查看明细的牌的原始 ID（可能是复制牌）
...
// ★ 辅助函数：将复制牌 ID 映射到母本 ID，保证组合明细查询用母本 ID
function resolveCardIdForCombos(cardId) {
    if (cardId.endsWith('_02')) {
        return cardId.replace('_02', '_01');
    }
    return cardId;
}
...
function onHandClick(cardId) {
    ...
    if (myTurn && !koikoi) {
        ...
        if (prev === cardId) {
            ...
            // 明细查看：映射到母本查询组合，但保留原始 ID 用于显示牌名
            const queryId = resolveCardIdForCombos(cardId);
            GameState.selectedDetailCard = cardId; // 记录原始 ID 用于显示名称
            GameState.selectedDetailCombos = getTheoreticalCombosForCard(queryId);
...
function onFieldClick(fieldCardId, index) {
    ...
    if (myTurn && !koikoi && GameState.selectedHandCard) {
        if (!canPair(GameState.selectedHandCard, fieldCardId)) {
            addLog('不能配对');
            // 不能配对时，仍可查看明细
            const queryId = resolveCardIdForCombos(fieldCardId);
...
function renderField() {
    ...
    GameState.fieldCards.forEach((fc, i) => {
        ...
        if (GameState.selectedDetailCard === topId && !(GameState.selectedHandCard && canPair(GameState.selectedHandCard, topId))) {
...
function renderRightSidebar() {
    ...
    for (const combo of allCombos) {
        ...
        if (combo.cards && combo.cards.length > 0) {
            // 使用映射后的母本 ID 匹配
            const queryId = resolveCardIdForCombos(GameState.selectedDetailCard);
            if (combo.cards.includes(queryId)) {
                covered = combo.cards;
            }
        } else {
            const queryId = resolveCardIdForCombos(GameState.selectedDetailCard);
            covered = getCoveredCardsForCombo(combo, allCardIds);
            if (!covered.includes(queryId)) {
                covered = [];
            }
        }
